...
export class TransactionsService {
  ...
  async createTransferInternalRecord(dto: CreateTransferInternalDto,authorization: string) {
    try {
      ...
      const des: Account[] = await this.accountService.getActivePaymentAccountById(dto.accountDesNumber);
      ...
      const otpTransferDto = new CreateTransferOtpDto(user.id, record.id, otpCode)

      const otpRes: Otp = await this.otpService.createTransferOtp(otpTransferDto)
  ...
  async verifyTransferInternalOtp (dto: VerifyTransferInternalDto, authorization: string) {
    try {
      const accessToken = this.userService.getAccessTokenFromClient(authorization);
      const decodedAccessToken = this.jwtService.decode(accessToken);
      ...
      const lastOtp = await this.otpService.getLastestOtpByTransactionOfCustomer(user.id,dto.transactionId)
      const currTime = new Date()
      currTime.setHours(currTime.getHours() - 7)
      const otpExpirationTime = parseInt(process.env.OTP_EXPIRATION_TIME || "60000")
      ...
      await this.accountService.updateBalanceById(transaction.accountSrcNumber,transaction.amount,TransactionType.TRANSFER)
      await this.accountService.updateBalanceById(transaction.accountDesNumber,transaction.amount,TransactionType.RECEIVE)
