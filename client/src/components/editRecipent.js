...
const EditRecipent = (props) => {
    ...
    async function editRecipent() {
        const info = await fetcherEditReceiver(props.editInfo.id, nickname, props.editInfo.name, props.editInfo.accNum)
    }
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex items-center justify-center">
            <div className="border-t-8 border-t-black relative flex-col justify-center bg-white rounded-xl w-2/5 ">
                ...
                <div className="flex  text-lg  text-black font-bold pt-4 px-20 border-b-2 border-b-gray-100" >Chỉnh sửa tên người nhận</div>
                ...
                <div className="flex justify-end pb-4 px-8">
                    ...
                    <button id="handleSave" onClick={handleSaveClick} className=" rounded px-4 py-2 ml-4 text-white  text-xs font-bold hover:bg-[#cf4a04] bg-[#EA580C] disabled:bg-[#edb395] "  >Save</button>
                </div>
            </div>
        </div>
    )
