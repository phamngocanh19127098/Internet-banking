...
import { fetcherListReceiversExternal, fetcherListReceiversInternal } from "../fetchers/fetcherCustomer";

const ListRecipentsExternal = (props) => {
    const { userInfo } = useSelector((state) => state.auth);
    const [indexSelect, setIndexSelect] = useState(0);
    const [listRecipents, setListRecipents] = useState([{}]);

    async function getList() {
        const list = await fetcherListReceiversExternal(userInfo.id);
    ...
    const handleAccount = (e) => {
        ...
        console.log(listRecipents[parseInt(e.target.value)].beneficiaryAccountNumber)
    };
    ...
    const handleSaveClick = (e) => {
        props.handleAccNum(listRecipents[parseInt(indexSelect)].beneficiaryAccountNumber);
        props.handleBank(listRecipents[parseInt(indexSelect)].beneficiaryBankId);
        props.onClose();
    };
    return (
...
export default ListRecipentsExternal;
