...
import { fetcherListReceiversExternal } from "../fetchers/fetcherCustomer";
...
const ListRecipentsExternal = (props) => {
  ...
  const handleAccount = (e) => {
    ...
    console.log(
      listRecipents[parseInt(e.target.value)].beneficiaryAccountNumber
    );
  };
  ...
  const handleSaveClick = (e) => {
    props.handleAccNum(
      listRecipents[parseInt(indexSelect)].beneficiaryAccountNumber
    );
            ...
            >
              {listRecipents.length !== 0
                ? listRecipents.map((account, index) => (
                    <option value={index}>{account.beneficiaryNickname}</option>
                  ))
                : null}
            </select>
          </div>
